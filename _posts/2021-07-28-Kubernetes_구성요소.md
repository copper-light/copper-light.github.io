...
* node
  * 서버 컴퓨터를 의미한다. 쿠버네티스는 기본적으로 다수의 서버 컴퓨터에서 운영되므로 이를 Node 라는 이름으로 관리하게 된다.
  * Node 는 각 컨테이너들과 구성요소들을 스케쥴링하고 관리하는 Control plane (Master)와 실질적으로 컨테이너들이 실행되어 운영되는 Worker 로 구분될 수 있다. Control plane 은 기본적으로 Worker 의 역할이 수행되지 않도록 설정되어 있으나 두개의 역할을 동시에 수행할 수 있도록 설정 가능하다.
  * 쿠버네티스의 최소 권장사항을 보면 Control plane 은 Worker 보다 1.5배의 CPU 자원 및 메모리를 요구한다.
...
* StatefulSet
	* 상태 유지가 필요한 Pod 를 운영할 때 사용하는 기능이다. 기존 Deployment 나 RelicaSet 은 상태 유지가 안된다는 특징이 있다. 다시 말해 Pod가 종료되면 아예 새로운 이름의 Pod로 대체된다는 것이다. 또한 스토리지도 모든 Pod 가 하나를 공유하게 된다. StatefulSet 은 이와 다르게 각 Pod 는 항상 동일한 이름을 갖게 되며 각 Pod 마다 각기 다른 스토리지를 갖게하는 기능이다.
...
* EndPoints
	* IP 를 담당하는 하는 기능이다. Service 를 선언하면 내부적으로 EndPoints를 생성해서 매핑해준다. 
	* 참고로 Service와 Pod 는 이후에 설명할 Label 로 매핑된다. 디테일하게 쿠버네티스의 서비스를 설정하고자 한다면 건드리겠지만 일반적으로 쓸일은 많지 않다.
...
* Label
	...
	* 다양한 조건을 걸어서 특정 Label 을 가진 Pod 들을 검색하는 것이 가능하며, Node 에도 Label이 있으므로 이를 통해 특정 Node 에 Pod를 실행하는 것도 가능하다.
...
### 마무리
일단 생각나는대로 마구 적어보았는데, 나중에 빠진게 있다싶으면 추가로 넣어 두겠다. - 끗 -
